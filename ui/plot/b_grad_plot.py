...
class BGradPlot(BasePlot):
    ...
    def update_image(self):
        ...
        if self._metric_l and self._metric_r and self._layer >= 0:
            l_grad = self._metric_l.b_gradient[self._layer]
            l_state = self._metric_l.b[self._layer]
            r_state = self._metric_r.b[self._layer]
            sum_grad = l_grad + l_state - r_state

        if sum_grad is not None:
            self._img.setImage(image=np.expand_dims(sum_grad, axis=1))
        else:
            self._img.clear()
